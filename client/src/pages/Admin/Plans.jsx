...
function Plans() {
  ...
  const cols = [
    ...
    {
      name: "Duaration",
      selector: (row) => `${row.dayDuaration} days`,
      sortable: true,
    },
    {
      name: "Description",
...

    {
      ...
      selector: (row) => (
        <>
          <button
            className="bg-blue-500 text-white px-2 py-1 rounded-full md:w-14 sm:w-14 h-11 md:h-10 text-sm md:text-base sm:text-lg"
            onClick={() =>
          ...
          <button
            ...
            } text-white px-2 py-1 rounded-full w-20 md:w-14 h-8 sm:w-14 md:h-10`}
            onClick={() => {
              deletePlan(row._id);
            }}
          >
  ...
  const getPlans = async () => {
    ...
    adminRequest({
      url: `${apiEndPoints.showPlans}?page=${currentPage + 1}`,
      method: "get",
    })
      .then((res) => {
